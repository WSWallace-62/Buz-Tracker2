 ...
 */
export const checkLongRunningTimers = onSchedule("every 60 minutes", async () => {
  ...
  try {
    ...
    for (const doc of querySnapshot.docs) {
      ...
      const thresholdHours = settings.reminderThresholdHours || 4; // Default to 4 hours if not set.
      const thresholdMillis = thresholdHours * 60 * 60 * 1000;

      // 3. Check if the session has been running longer than the user's threshold.
      if (now - session.startTs > thresholdMillis) {
        logger.info(`Session for user ${userId} on project "${session.projectName}" has been running for over ${thresholdHours} hours.`);
        ...
        if (tokensSnapshot.empty) {
          logger.warn(`User ${userId} has a long-running timer but no FCM tokens to notify.`);
          continue;
        }
        ...
        const messagePayload = {
          notification: {
            ...
            body: `Your timer for "${session.projectName}" has been active for over ${thresholdHours} hours.`
          },
          tokens: tokens,
        };
        ...
        try {
          ...
          logger.info(`Successfully sent message to ${response.successCount} device(s) for user ${userId}.`);
          if (response.failureCount > 0) {
            logger.warn(`Failed to send to ${response.failureCount} device(s) for user ${userId}.`);
        ...
        logger.info(`Session for user ${userId} is within the ${thresholdHours}-hour threshold.`);
      }
    }
  } catch (error) {
    logger.error("A critical error occurred while querying for long-running timers:", error);
  }
});
